module.exports = {
  /**
   * 查询多条记录 分页
   * @url client/charts/sys/getWord 前端调用的url参数地址
   * data 请求参数 说明
  ...
  main: async (event) => {
    ...
    // 业务逻辑开始----------------------------------------------------------- 
		...
		res = await vk.baseDao.selects({
			  ...
			  groupJson: {
				_id: "$reason", // _id是分组id， $ 后面接字段名，如按date_str字段进行分组(date_str字段是2021-08-19这样的字符串)
				count: _.$.addToSet("$reason"), // $ 后面接字段名，如把user_id原样输出（去重）
			  },
